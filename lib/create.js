...
module.exports = {
  ...
  volumes: (data) => {
    ...
    name.forEach((val, k) => {
      ...
      // plugin
      if (v.type == "plugin") {
        console.log("plugin s3fs enabled with driver ", v.driver);
        if (!v.target) {
          console.log("Plugin volume isn't valid!");
          if (!utils.exit(0)) return;
        }

        var set_plugin_alias = "docker plugin install --alias "+v.settings.alias+":latest "+v.driver+ ":latest --grant-all-permissions AWSACCESSKEYID="+v.settings.aws_key+" AWSSECRETACCESSKEY="+v.settings.aws_secret+" DEFAULT_S3FSOPTS="+v.settings.s3fsopts;
        // console.log(set_plugin_alias);
        chp.exec(set_plugin_alias, (error, stdout, stderr) => {
            if (error) {
        ...
        var docker_volume_create = "docker volume create -d "+v.settings.alias+":latest "+v.settings.bucket;
        // console.log(docker_volume_create);
        chp.exec(docker_volume_create, (error, stdout, stderr) => {
            if (error) {
                console.log(error.message);
                if (!utils.exit(0)) return;
            }
            ...
            console.log("volume plugin created", stdout);
        });

        volumes[name[k]] = "type=volume,src=" + v.settings.bucket + ",dst=" + v.target+",volume-driver=" + v.settings.alias +":latest";
      }
    });
  },
